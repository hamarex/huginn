  ...
  return {
    ...
    preProcessing: function (op, callback) {
      ...
      /* 上記 「データ配列の1行目」と「1列目」を項目名として取り出した結果、
        ...
        X軸ラベルのタイトル  huginn.colNamesTitle //"年度"
        X軸ラベルの配列    huginn.colNames
                //[2007, 2008, 2009, 2010, 2011, 2012, 2013]
        Y軸ラベルの配列    huginn.rowNames
                //["紅茶", "コーヒー", "ジュース", "ウーロン"]
      */
...
//            this.roundDigit = 3;//デフォルト値 0.12345は0.123 1や0.1や0.12 は、1や0.1や0.12。
            this.roundDigit = 1;//デフォルト値 0.12345は0.123 1や0.1や0.12 は、1や0.1や0.12。
          } else {
...
//      var titleY = this.op.config.titleY || this.gcf.titleY || 38;
    ...
    ws: function (url, op) {
      ...
      function _w(first) {
        ...
        that.wsuids[uid].on('open', function () {
          ...
          if (op.autoReConnect)
            if(that.wsReCnt['-huginn-ws-'+id+'-'+url]===undefined){
              if(!that.ops[id].wsReConnecting)
                that.wsReCnt['-huginn-ws-'+id+'-'+url] = 0; //自動接続数リセット
            }
          _hbTimerFnc(target);
        });
      ...
      function _autoReConnect(target) {
        ...
        // 接続回数はthat.wsReCnt['-huginn-ws-'+id+'-'+url]でカウントされます。
    ...
    wscase: {
      ...
      someColsAtATime: function (msg) {
       ...
       // huginn.init(that.id, that.op); //再起へ
    ...
    moduleExtend: function (oj, prop, name, update) {
      ...
      for (var i in prop) {
        if (i === 'aboutThis_module') {
          //aboutThis_moduleはhuginn.aboutThis.plugins以下へ
          oj['aboutThis']['plugins'][prop[i].name] = prop[i];
        } if (i === 'init') {
          //initはコピーしない
        } else {
          if(!oj[i] || update){
            //iがhuginn下に存在しないか、updateがtrueならコピーする
            oj[i] = prop[i];
          } else {
            for(var j in prop[i]){
    ...
    base: function base(){
      //Webパージ全体に係るチャートのグローバルデフォルト値huginn.gcfを設定する
      var that = this;
      ...
      if(len === 0){
        ...
        for(var i = 0; i < len; i++){
          var arg = arguments[i];
          //引数が '' か nullなら現在のhuginn.gcfをクリアし次の引数を適用する
          if(arg==='' || arg === null){
            this.gcf = {};
            continue;
          }

          //現在のhuginn.gcfへ_opNewを上書きする
  ...
  return {
    ...
    _css_arc: function(op){
      ...
      function showTooltip(id, e, op){
        ...
        //.-huginn-ttip-dataなどでCSSから指定可能です
